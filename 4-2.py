...
def findMostSleepingMin(guard):
    ...
    count = all_mins.count(most_common_min)
    return most_common_min, count

def getMostAsleepByGuard(guard_info):
    sleeper_dict = {}
    for guard_id in guard_info:
        most_sleep_min, count = findMostSleepingMin(guard_info[guard_id])
        sleeper_dict[guard_id] = { most_sleep_min : count }
    return sleeper_dict

def getSnoozerTime(snoozer_dict):
    for i in snoozer_dict[1].keys():
        time = i
    return time
...
if __name__ == '__main__':
    ...
    sleeper_dict = getMostAsleepByGuard(guard_info)
    snoozer_dict = max(sleeper_dict.items(), key=operator.itemgetter(0))
    snoozer_id = snoozer_dict[0]
    snoozer_time = getSnoozerTime(snoozer_dict)
    result = int(snoozer_id) * snoozer_time
    print(result)
